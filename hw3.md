...
SELECT AVG(pg_column_size(random_text)) AS avg_row_size_in_bytes
FROM tests.test_text AS t; --- 58 байт
```

58 * 11 (11 версий строк) * 1 000 000 = 638 000 000 байт = 638 МБ это просто данные 
...
Я так понимаю разница между реальным размером таблицы, вычисленным путем подсчета строк*размер, достигается все таки из-за фрагментации? Тостовые страницы не создаются...
